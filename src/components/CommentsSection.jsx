...
import { useToast } from '../components/ToastContext'; // <--- IMPORT
...
export default function CommentsSection({ postId, session, isAdmin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      addToast("Erreur lors de l'envoi : " + error.message, 'error');
    } else {
      setNewComment('');
      await fetchComments();
      addToast("Commentaire publié !", 'success'); // <--- TOAST
    }
    setSending(false);
  };
  ...
  const confirmDelete = async () => {
    ...
    if (error) {
      ...
      addToast("Commentaire supprimé", 'info'); // <--- TOAST
    }
        ...
        {loading ? (
          ...
          comments.map(comment => (
            <div key={comment.id} className="flex gap-3 group">
              <div className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center shrink-0 font-bold uppercase text-xs border overflow-hidden ${comment.profiles?.role === 'admin' ? 'bg-purple-500/20 text-purple-400 border-purple-500/30' : 'bg-wiki-surface text-wiki-muted border-wiki-border'}`}>
